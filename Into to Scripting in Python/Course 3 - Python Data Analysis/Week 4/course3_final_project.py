...
Analyzing baseball statistics in CSV files
"""

import csv

#
# Code from week 3 project for reading csv files
#
...
#
# Formulas for common batting stats (given by instructor)
#
...
def filter_by_year(statistics, year, yearid):
    ...
    for a_dict in statistics:
        if a_dict[yearid] == str(year):
            table.append(a_dict)
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for a_dict in statistics:
        table.append(
            (a_dict[info['playerid']], formula(info, a_dict))
        )

    table = sorted(table, key=lambda pair: pair[1], reverse=True)
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    master_stats = read_csv_as_list_dict(
        info['masterfile'],
        info['separator'],
        info['quote']
    )
    ...
    for player_id in top_ids_and_stats:
        for a_dict in master_stats:
            if a_dict[info['playerid']] == player_id[0]:
                table.append(
                    '{:.3f} --- {} {}'.format(
                        player_id[1],
                        a_dict[info['firstname']],
                        a_dict[info['lastname']]
                    )
                )
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    filter_year = filter_by_year(batting_stats, year, info['yearid'])
    top_player = top_player_ids(info, filter_year, formula, numplayers)
    player_names = lookup_player_names(info, top_player)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for a_dict in statistics:
        final_dict[a_dict[playerid]] = {}
        for field in fields:
            final_dict[a_dict[playerid]][field] = 0
            final_dict[a_dict[playerid]][playerid] = a_dict[playerid]

    for key in final_dict:
        for a_dict in statistics:
            if key == a_dict[playerid]:
                for field in fields:
                    final_dict[key][field] += int(a_dict[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    player_stats = aggregate_by_player_id(
        batting_stats,
        info['playerid'],
        fields
    )

    players_names = [key for key, value in player_stats.items()]

    stats_list = []

    for player in players_names:
        stats_list.append((player, formula(info, player_stats[player])))

    stats_list = sorted(
        stats_list, key=lambda pair: pair[1], reverse=True
    )

    return lookup_player_names(info, stats_list[0: numplayers])
